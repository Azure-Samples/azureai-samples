...
module storageAccessAssignment './storage-role-assignments.bicep' = if(!aiStorageExists){
  name: 'dependencies-storage-rbac'
  params: {
    storageName: storageNameCleaned
    UAIPrincipalId: userAssignedIdentity.principalId
    }
  dependsOn: [ storage ]
}

module keyVaultAccessAssignment './keyvault-role-assignments.bicep' = {
  name: 'dependencies-keyvault-rbac'
  params: {
